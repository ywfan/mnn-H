...
class CheckRelError(Callback):
    def __init__(self, X_train, Y_train, X_test, Y_test,
                verbose = False, period=1, errorFun = rel_error):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if((epoch)%self.period == 0):
            ...
            err_train = self.errorFun(self.model, self.X_train, self.Y_train)
            err_test  = self.errorFun(self.model, self.X_test, self.Y_test)
